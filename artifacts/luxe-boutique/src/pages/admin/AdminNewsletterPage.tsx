...
type Campaign    = { id: string; subject: string; body: string; recipientCount: number; sentCount: number; status: string; sentAt: string };
type NewsletterData = {
...
export default function AdminNewsletterPage() {
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (d) => {
      setSent({ count: d.recipientCount });
      setSubject("");
      setBody("");
      setPreview(false);
              ...
              {sent && (
                <div className="mb-6 p-4 bg-[#e6f7f1] border border-[#6cf8bb]/40 rounded-xl flex items-center gap-3">
                  ...
                  <p className="text-sm font-[Manrope] font-bold text-[#006c49]">
                    Campaign sent to {sent.count} subscriber{sent.count !== 1 ? "s" : ""}!
                  </p>
                </div>
              )}
