...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [asset] = await db.select().from(mediaAssets).where(eq(mediaAssets.id, req.params.id as string)).limit(1);
    ...
    await db.delete(mediaAssets).where(eq(mediaAssets.id, req.params.id as string));
